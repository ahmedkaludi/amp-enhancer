...
class Amp_Toggle extends Widget_Base {
	...
	protected function render() {
		...
		?>
		<amp-accordion class="elementor-toggle" role="tablist" animate disable-session-states>
			<?php
			foreach ( $settings['tabs'] as $index => $item ) :
				...
				$this->add_inline_editing_attributes( $tab_content_setting_key, 'advanced' );
                 $sign_icon = '';
                  if ( $has_icon ) :
					$sign_icon = '<span class="elementor-toggle-icon elementor-toggle-icon-'.esc_attr( $settings['icon_align'] ).'" aria-hidden="true">';							
					    if ( $is_new || $migrated ) {  
						  $sign_icon  .=	'<i class="elementor-toggle-icon-closed '.esc_attr( $settings['selected_icon']['value']  ).'"></i>
								<i class="elementor-toggle-icon-opened  '.esc_attr( $settings['selected_active_icon']['value'] ).'"></i>';
							 } else { 
							$sign_icon  .=	'<i class="elementor-toggle-icon-closed '.esc_attr( $settings['icon']  ).'"></i>
				...
				?>
				<section class="elementor-toggle-item">
					<h4 class="elementor-toggle-title elementor-tab-title elementor-toggle-icon-<?php echo esc_attr( $settings['icon_align'] ); ?>" aria-hidden="true">
						   
						<?php echo $sign_icon; echo esc_html($item['tab_title']); ?>
