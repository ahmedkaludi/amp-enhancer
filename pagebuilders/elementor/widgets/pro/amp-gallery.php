<?php
namespace Amp_Enhancer_Elementor_Support\Widgets\Pro;
...
class AMP_Gallery extends Base_Widget {
	...
	protected function render() {
		...
		if ( $is_multiple ) {
			...
			<div data-id="<?php esc_attr_e($id); ?>" <?php echo $this->get_render_attribute_string( 'titles-container' ); ?>>
        ...
        $gallery_grid = 'e-gallery-grid';
		/*if(isset($settings['gallery_layout']) && $settings['gallery_layout'] == 'justified'){
			$gallery_grid = 'e-gallery-justified';
		}elseif(isset($settings['gallery_layout']) && $settings['gallery_layout'] == 'masonry'){
			$gallery_grid = 'e-gallery-masonry';
		}*/

		$this->add_render_attribute( 'gallery_container', 'class', 'elementor-gallery__container e-gallery-container '.esc_attr($gallery_grid).'  e-gallery--ltr' );
		...
		if(isset($settings['columns'])){
			...
			$col_css = '--columns:'.esc_attr($columns).';';
		}
		$aspect_css = '--aspect-ratio:75%;';
        if(isset($settings['aspect_ratio'])){
        	$ratio = explode(':', $settings['aspect_ratio']);
        	$aspect_ratio = round( ((100/$ratio[0])*$ratio[1]),3);
        	$aspect_css = '--aspect-ratio:'.esc_attr($aspect_ratio).'%;';
        }

		if ( ! empty( $galleries ) ) { ?>
		<div id="eg<?php esc_attr_e($id); ?>" <?php echo $this->get_render_attribute_string( 'gallery_container' ); ?> style="--hgap: <?php esc_attr_e($gap); ?>px;--vgap: <?php esc_attr_e($gap); ?>px;--animation-duration: 350ms;<?php echo $col_css; ?>--rows: 3;<?php 
		    echo $aspect_css; ?>--container-aspect-ratio: 50.6452%;">
			<?php
			foreach ( $gallery_items as $id => $tags ) :
				...
				<<?php echo $gallery_item_tag; ?> <?php echo $this->get_render_attribute_string( 'gallery_item_' . $unique_index ); ?>>
					<div  <?php echo $this->get_render_attribute_string( 'gallery_item_image_' . $unique_index ); ?> style="background-image:url(<?php echo esc_url($image_data['src']); ?>)" ></div>
