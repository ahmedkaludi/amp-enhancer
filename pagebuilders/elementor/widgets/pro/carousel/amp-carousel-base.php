<?php
namespace Amp_Enhancer_Elementor_Support\Widgets\Pro\Carousel;
...
abstract class Base extends Base_Widget {
	...
	protected function print_slider( array $settings = null ) {
		...
		$carousel_type = $this->get_name();
		$default_slides = 3;
		if($carousel_type == 'testimonial-carousel'){
			$default_slides = 1;
		}
		$important = '';
		$slides_per_view = (isset($settings['slides_per_view']) &&  !empty($settings['slides_per_view']) ) ? $settings['slides_per_view'] : $default_slides;
		if(isset($settings['skin']) && $settings['skin'] == 'slideshow'){
			$slides_per_view = 1;

			if(isset($settings['container_class']) && $settings['container_class'] == 'elementor-thumbnails-swiper'){
				$slides_per_view = (isset($settings['slideshow_slides_per_view']) &&  !empty($settings['slideshow_slides_per_view']) ) ? $settings['slideshow_slides_per_view'] : 5;
				$important = '!important';
			}
		}


		$slides_width = round(100/$slides_per_view,3);


/*		if($carousel_type == 'testimonial-carousel'){
		$slides_width = round(580/$slides_per_view,3);
		}*/
		...
		$slides_scroll = (isset($settings['slides_to_scroll']) &&  !empty($settings['slides_to_scroll']) ) ? $settings['slides_to_scroll'] : 1;
			...
			$dots = false;
			if(isset($settings['pagination']) && $settings['pagination'] == 'bullets'){
				$dots = true;
			}
		?>
		<div class="elementor-swiper">
			<div class="<?php echo esc_attr( $settings['container_class'] ); ?> swiper-container  swiper-container-horizontal">
				<div class="swiper-wrapper swiper-wrapper<?php esc_attr_e($id); ?>" data-transform-px="0" data-frame="0">
					<?php
					$l = 1;
					foreach ( $settings['slides'] as $index => $slide ) :
						$this->slide_prints_count++;
						$k = $l-1;
						$transform_width = (100)*($l-1);
						$thumb_transform = ($slides_width*$no_scroll)*($l-1);
						?>
						<div class="swiper-slide" style = "width:<?php esc_attr_e($slides_width);?>% <?php esc_attr_e($important); ?>" data-id="<?php esc_attr_e($id); ?>" data-total-slides="<?php esc_attr_e($slides_count); ?>" data-slide-width="<?php esc_attr_e($slides_width); ?>"  data-transform="<?php esc_attr_e($transform_width); ?>"
						data-thumbtransform="<?php esc_attr_e($thumb_transform); ?>" data-bullet="<?php esc_attr_e($k); ?>" >
